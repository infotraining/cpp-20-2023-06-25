...
template <typename Container>
concept StdSizedContainer = StdContainer<Container> && requires(Container& c) {
...
// concept StdContainer = requires(Container& c)
// {
//     { std::begin(c) } -> std::same_as<typename std::ranges::iterator_t<Container>>;
//     { std::end(c) } -> std::same_as<typename std::ranges::iterator_t<Container>>;
//     { std::size(c) } -> std::convertible_to<size_t>;
// };
...
template <typename Container>
concept Indexable = requires(Container& c, Index_t<Container> index)
{
    c[index];
};

template <typename Container>
concept IndexableContainer = StdSizedContainer<Container> && Indexable<Container>;
...
TEST_CASE("concepts")
{
    ...
    static_assert(IndexableContainer<std::unordered_map<std::string, int>>);
